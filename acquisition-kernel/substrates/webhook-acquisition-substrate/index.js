 ...
 * @returns {Promise<{ processed: number, discarded: number, results: Array }>}
 */
async function _processEntry(entry, accountId) {
  ...
  if (!entry || typeof entry !== 'object') {
    return { processed, discarded, results: [{ status: 'discarded', reason: 'entry_not_object' }] };
  }
  ...
  if (Array.isArray(entry.messaging) && entry.messaging.length > 0) {
    for (const item of entry.messaging) {
      ...
      results.push(r);
      if (r.status === 'staged') processed++;
      else discarded++;
    }
    return { processed, discarded, results };
  }
