// dedup: transitions → { currentDedupState, batchMarks, batchReplays, collisionCount }
const PROJECTION_VERSION = '1.0.0';
const DEDUP_FSM_STATES = Object.freeze(['IDLE', 'BATCHING', 'MARKING', 'REPLAYING', 'COMPLETED', 'FAILED']);
...
function synthesize(_projectionState, signals) {
  ...
  const batchMarks = deriveLastValue(safeTransitions, 'batchMarks') || 0;
  const batchReplays = deriveLastValue(safeTransitions, 'batchReplays') || 0;

  return {
    currentDedupState: currentState,
    batchMarks,
    batchReplays,
    collisionCount: countCollisions(safeTransitions),
...
function deriveCurrentState(transitions) {
  ...
  for (let i = transitions.length - 1; i >= 0; i--) {
    ...
    if (t.entity === 'fsm' && DEDUP_FSM_STATES.includes(t.nextState)) return t.nextState;
    if (t.domain === 'dedup' && t.entity === 'fsm' && t.nextState) return t.nextState;
  }
  return 'IDLE';
}
...
function countCollisions(transitions) {
  let count = 0;
  for (const t of transitions) {
    const intent = (t.raw && t.raw.intent) || '';
    if (intent === 'DEDUP_REPLAY_DETECTED' || intent === 'CHECK_AND_MARK_DEDUP') count++;
  }
  return count;
}
