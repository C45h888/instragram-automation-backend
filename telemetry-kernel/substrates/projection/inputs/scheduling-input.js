// telemetry-kernel/substrates/projection/inputs/scheduling-input.js
// Scheduling Projection Input: reads the scheduling FSM transition log slice.
...
async function getNormalizedInputWindow({
  ...
  if (observability && observability.query && typeof observability.query.getTransitionLog === 'function') {
    try {
      const log = observability.query.getTransitionLog('scheduling', null, windowSize);
  ...
  return {
    ...
    source: 'observability.transitionLog[scheduling]',
  };
}
