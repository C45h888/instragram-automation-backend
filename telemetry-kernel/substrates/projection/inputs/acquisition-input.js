...
async function getNormalizedInputWindow({ pollIntervalMs = DEFAULT_POLL_INTERVAL_MS, tickCount = 0, windowSize = DEFAULT_WINDOW_SIZE } = {}) {
  const observability = readObservability(); const now = Date.now();
  let transitions = [];
  if (observability && observability.query && typeof observability.query.getTransitionLog === 'function') {
    try { const log = observability.query.getTransitionLog('acquisition', null, windowSize); transitions = Array.isArray(log) ? log : []; }
    catch (_) { transitions = []; }
  }
  return { transitions, now, tickCount, windowOpenedAt: now - pollIntervalMs, windowClosedAt: now, entryCount: transitions.length, noiseGate: transitions.length < 3, source: 'observability.transitionLog[acquisition]' };
}
