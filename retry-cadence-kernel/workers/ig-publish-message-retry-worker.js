// retry-cadence-kernel/workers/ig-publish-message-retry-worker.js
// IG Publish Message Retry Worker — re-invokes the publish:message
// operation via the engagement substrate.

const engagementSubstrate = require('../../publishing-kernel/substrates/engagement');

const SUPPORTED_DOMAINS = ['publish:message'];
...
async function execute(event, governance) {
  ...
  if (!SUPPORTED_DOMAINS.includes(domain)) {
    return {
      success: false,
      workerName: 'ig-publish-message-retry-worker',
      durationMs: 0,
  ...
  if (result && result.success) {
    if (governance?.dispatchGlobal) {
      governance.dispatchGlobal({
        type: 'IG_PUBLISH_MESSAGE_RETRY_COMPLETE',
...
module.exports = {
  name: 'ig-publish-message-retry-worker',
  execute,
