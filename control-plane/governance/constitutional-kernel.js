...
// ═══════════════════════════════════════════════════════════════════════════════
// 10. Watchdog — state staleness detection + capability cadence tick
// ═══════════════════════════════════════════════════════════════════════════════
// Two intervals managed as a single loop infrastructure:
//   - watchdog tick (10s): checks state TTL, auto-recovers from DEGRADED/BOOTING/RECOVERY
//   - cadence tick (1h): dispatches CAPABILITY_CADENCE_TICK to GCK FSM
...
function startLoop(intervalMs = 10_000) {
  ...
  _watchdogInterval = setInterval(_watchdogTick, intervalMs);
  _watchdogInterval.unref();
  console.log(`[constitutional-kernel] Watchdog loop started — tick every ${intervalMs}ms`);

  // Also start the capability cadence tick
  if (!_cadenceInterval) {
    _cadenceInterval = setInterval(_cadenceTick, CADENCE_TICK_INTERVAL_MS);
