// Phase 9 — Drift: semantic
// public_signal vs internal sentinel
...
describe('drift/semantic-drift', () => {
  ...
  beforeAll(async () => {
    harness = new p9.RuntimeHarness({ runId: 'p9-drift-semantic' });
    await harness.boot();
  ...
  it('no semantic drift detected in a clean run', async () => {
    // Run a small chain through the runtime.
    const correlationId = `p9-drift-semantic-${Date.now()}`;
    harness.injectEvent({
