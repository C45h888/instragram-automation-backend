...
//
// Asserts the owner of every link in the chain for the conversation
...
const FIXTURE_PATH = path.join(__dirname, '..', 'fixtures', 'webhooks', 'canonical', 'conversation-update.json');

describe('ownership/ownership-trace-conversation', () => {
  let harness;
  let writer;

  beforeAll(async () => {
    harness = new p9.RuntimeHarness({ runId: 'p9-ownership-conversation' });
    await harness.boot();
    writer = new p9.ReportWriter({ reportDir: harness.reportDir, runId: harness.runId });
  }, 60000);

  afterAll(async () => {
    writer.writeSummary('ownership', 'ownership-trace-conversation');
    if (harness) await harness.shutdown();
  }, 30000);

  it('chain owners match the architecture mandate', async () => {
    const body = JSON.parse(fs.readFileSync(FIXTURE_PATH, 'utf8'));
    const correlationId = `p9-own-conversation-${Date.now()}`;
    harness.injectEvent({
      type: 'WEBHOOK_DELIVERED',
      source: 'runtime/ingress',
      payload: { fixture: 'conversation', body },
      correlationId,
    });
