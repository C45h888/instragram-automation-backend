// Phase 9 — Tier 2 Graph Worker: recovery.
import { describe, it, expect, beforeAll, afterAll } from 'vitest';
import p9 from '../runtime/index.mjs';

describe('graph/runtime-graph-recovery — Tier 2', () => {
  let harness;
  let writer;

  beforeAll(async () => {
    harness = new p9.RuntimeHarness({ runId: 'p9-graph-recovery' });
    await harness.boot();
    writer = new p9.ReportWriter({ reportDir: harness.reportDir, runId: harness.runId });
  }, 60000);

  afterAll(async () => {
    writer.writeSummary('graph', 'runtime-graph-recovery');
    if (harness) await harness.shutdown();
  }, 30000);

  it('recovery worker executes and the chain is constitutional', async () => {
    const correlationId = `p9-recovery-${Date.now()}`;
    harness.injectEvent({
      type: 'RECOVERY_REQUESTED',
      source: 'graph/recovery',
      payload: { kind: 'token-refresh' },
      correlationId,
    });
    await harness.tick(3);

    const snap = harness.snapshotDeriver.derive();
    const event = snap.events[correlationId];
    expect(event, 'recovery event not observed').toBeDefined();
    writer.bumpAssertions(1);
  });
});
