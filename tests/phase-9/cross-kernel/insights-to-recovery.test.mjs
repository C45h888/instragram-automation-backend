...
// Source: insights  →  Sink: recovery
//
// Validates: the runtime's CK routes the cross-kernel signal from
// insights to recovery constitutionally. The recorder-observer captures
...
describe('cross-kernel/insights-to-recovery', () => {
  it('routes the signal constitutionally', async () => {
    const r = await runPair({ source: 'insights', sink: 'recovery' });
