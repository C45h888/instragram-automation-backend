...
// Kernel: recovery
//
// Validates the recovery kernel can:
//   1. Boot with CK in HEALTHY state (no degraded-mode errors)
//   2. Accept RECONCILIATION_TICK events without rejections
...
describe('sovereignty/kernel-sovereignty-recovery', () => {
  ...
  it('reconciliation domain event routes through CK without unexpected rejections', async () => {
    ...
    harness.simulator.injectEvent({
      ...
      correlationId: `p9-sov-recovery-${Date.now()}`,
    });
  ...
  it('lineage ledger has entries after reconciliation event injection', async () => {
    ...
    expect(snap.lineage.size, 'lineage is empty after recovery kernel boot — events not written to ledger').toBeGreaterThan(0);
    writer.bumpAssertions(1);
  });
});
