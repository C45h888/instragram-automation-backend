...
// Kernel: capability
//
// Validates the capability kernel can:
//   1. Boot with CK in HEALTHY state (no degraded-mode errors)
//   2. Accept capability domain events without rejections
//   3. Show no unexpected governance rejections in the log

import { describe, it, expect, beforeAll, afterAll } from 'vitest';
import p9 from '../runtime/index.mjs';

describe('sovereignty/kernel-sovereignty-capability', () => {
  let harness;
  let writer;

  beforeAll(async () => {
    harness = new p9.RuntimeHarness({ runId: 'p9-sov-capability' });
    await harness.boot();
    writer = new p9.ReportWriter({ reportDir: harness.reportDir, runId: harness.runId });
  }, 60000);

  afterAll(async () => {
    writer.writeSummary('sovereignty', 'kernel-sovereignty-capability');
  ...
  it('capability domain event routes through CK without unexpected rejections', async () => {
    ...
    harness.simulator.injectEvent({
      ...
      correlationId: `p9-sov-capability-${Date.now()}`,
    });

    const newDecisions = harness.simulator.governanceLog().slice(decisionsBefore);
    expect(newDecisions.length, 'no governance decision fired for CAPABILITY_BOOTSTRAP').toBeGreaterThan(0);

    const rejections = newDecisions.filter((d) => d.result === false || d.result?.allowed === false);
    expect(rejections, `CK rejected CAPABILITY_BOOTSTRAP: ${JSON.stringify(rejections)}`).toHaveLength(0);
    writer.bumpAssertions(2);
  });

  it('lineage ledger has entries after capability event injection', async () => {
    const snap = await harness.simulator.snapshot();
    expect(snap.lineage, 'lineage missing from snapshot').toBeDefined();
    expect(snap.lineage.size, 'lineage is empty after capability kernel boot — events not written to ledger').toBeGreaterThan(0);
    writer.bumpAssertions(1);
  });
});
